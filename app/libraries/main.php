...
class MEC_main extends MEC_base
{
    ...
    public function save_event($event = [], $post_id = null)
    {
        ...
        if (!$post_id) return new WP_Error('mec_event_insert_failed', esc_html__('The event could not be created.', 'mec'), ['status' => 500]);
        ...
        if (!$mec_event_id)
        {
            ...
            if (!$result)
            {
                return new WP_Error('mec_event_record_insert_failed', ($wpdb->last_error ? $wpdb->last_error : esc_html__('The event schedule record could not be created.', 'mec')), ['status' => 500]);
            }
        }
        else
        {
            ...
            if ($result === false)
            {
                return new WP_Error('mec_event_record_update_failed', ($wpdb->last_error ? $wpdb->last_error : esc_html__('The event schedule record could not be updated.', 'mec')), ['status' => 500]);
            }
        }
    ...
    public function get_integrated_plugins_for_import()
    {
        return [
            ...
            'bookly' => esc_html__('Bookly', 'mec'),
            'amelia' => esc_html__('Amelia', 'mec'),
