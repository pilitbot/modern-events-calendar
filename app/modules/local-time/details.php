...
<div class="mec-local-time-details mec-frontbox" id="mec_local_time_details">
    <?php echo ((isset($icons) && $icons) ? $icons->display('clock') : '<i class="mec-sl-clock"></i>'); ?><h3 class="mec-local-time mec-frontbox-title"><?php esc_html_e('Local Time', 'mec'); ?></h3>
    <ul>
        <li><?php echo sprintf(esc_html__('Timezone: %s', 'mec'), '<span>'.esc_html($timezone).'</span>'); ?></li>
        <li><?php echo sprintf(esc_html__('Date: %s', 'mec'), $this->date_label(array('date'=>date('Y-m-d', $user_start_time)), array('date'=>date('Y-m-d', $user_end_time)), $date_format1)); ?></li>
        <?php if(!$hide_time and trim($time_format)): ?>
        <li><?php echo sprintf(esc_html__('Time: %s', 'mec'), '<span>'.($allday ? $this->m('all_day', esc_html__('All Day' , 'mec')) : ($hide_end_time ? date($time_format, $user_start_time) : date($time_format, $user_start_time).' - '.date($time_format, $user_end_time))).'</span>'); ?></li>
        <?php endif; ?>
    </ul>
</div>
