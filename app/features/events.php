...
class MEC_feature_events extends MEC_base
{
    ...
    {
        // Event Details
        add_meta_box('mec_metabox_details', esc_html__('Details', 'mec'), [$this, 'meta_box_details'], $this->main->get_main_post_type(), 'normal', 'high');
    ...
    public function action_links($actions, $post)
    {
        ...
        if ($this->getPRO() && $can_access_attendees && isset($this->settings['booking_status']) && $this->settings['booking_status']) $actions['mec-bookings'] = '<a href="' . esc_url($this->main->add_qs_vars([
            ...
            'page' => 'mec-attendees',
            'event_id' => $post->ID,
        ], trim($this->main->URL('admin'), '/ ') . '/edit.php')) . '">' . esc_html__('Attendees', 'mec') . '</a>';
